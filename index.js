...
function memoizedArea() {
  ...
  // Повертаємо площу
  let prevSide = null; // Зберігає попередню сторону квадрата
  let prevResult = null; // Зберігає попередній результат

  return function(side) {
    if (prevSide === side) {
      console.log("Fetching from cache");
      return prevResult; // Повертаємо результат з кешу
    } else {
      console.log("Calculating result");
      const result = side * side; // Обчислюємо площу квадрата
      prevSide = side; // Зберігаємо аргумент для майбутнього використання
